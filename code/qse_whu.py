...
class WHUDataset(Dataset):
    def __init__(self, cloudy_dir, clear_dir, transform=None):
        ...
        assert len(self.cloudy_images) == len(self.clear_images), \
            "Number of cloudy and clear images must match"
...
class QSEBlock(nn.Module):
    """
    Quantum Squeeze-and-Excitation:
      - Excite: amplitude-encode -> VQC [(RY, CNOT)*n_layers] -> PauliZ expvals -> Linear -> Sigmoid
    Notes:
      - Uses amplitude encoding; input length must be 2^n. We pad if channels < 2^n.
      - QNode runs on CPU; we quietly move the pooled vector to CPU for the quantum call
        and bring the result back to the original device. Autograd works end-to-end.
    """
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        assert c == self.channels, "Channel mismatch in QSEBlock"
        ...
        q_out = torch.stack(q_out, dim=0).to(x.dtype)  # [B, n_qubits]

        y_excite = self.act(self.fc_out(q_out))  # [B, C]
...
class UNetBlock(nn.Module):
    def __init__(self, in_ch, out_ch, time_emb_dim):
        ...
        self.qse = QSEBlock(out_ch, n_layers=2) # <-- inserted QSEBlock here
        self.act2 = nn.SiLU()
    ...
    def forward(self, x, t):
        ...
        x = self.qse(x)      # <-- apply QSE attention here
        x = self.act2(x)
...
if __name__ == "__main__":
    print(device)
    # Paths
    # train_cloud_dir = "../data/WHU/train/cloudy"
    # train_label_dir = "../data/WHU/train/clear"

    # test_cloud_dir = "../data/WHU/test/cloudy"
    # test_label_dir = "../data/WHU/test/clear"

    # print("Loading WHU Cloud Dataset...")

    # train_dataset = WHUDataset(train_cloud_dir, train_label_dir, transform=image_transforms)
    # val_dataset = WHUDataset(test_cloud_dir, test_label_dir, transform=image_transforms)
